...
class CardScript : MonoBehaviour
{
    ...
    void OnMouseEnter()
    {
        if (GameData.gameState != States.BattleEnded)
        {
            ...
            DescText.text = GameData.CardDetailsToString(actualCard);
        }
    }
    ...
    void OnMouseUp()
    {
        ...
        if (canLand && GameData.Player.Mana >= actualCard.Cout && GameData.gameState == States.PlayerTurn)
        {
            CardReader.BroadcastMessage("ActionCard", actualCard);
            Destroy(gameObject);
        }
        else if(canLand && GameData.Player.Mana < actualCard.Cout || GameData.gameState != States.PlayerTurn)
        {
